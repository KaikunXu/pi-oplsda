...
def dummy_data() -> tuple[pd.DataFrame, pd.Series]:
    """Generate a small synthetic metabolomics dataset for tests."""
    np.random.seed(42)
    n_samples = 20
    n_features = 15
...
def test_model_fit_and_basic_metrics(
    dummy_data: tuple[pd.DataFrame, pd.Series],
) -> None:
    """Validate basic fitting and fitted metric attributes."""
...
def test_compute_q2_and_summary_df(
    dummy_data: tuple[pd.DataFrame, pd.Series],
) -> None:
    """Validate Q2 computation and component summary export."""
    df_X, df_y = dummy_data
    model = OPLSDA(n_ortho=1, cv_folds=3)
    model.fit(df_X, df_y)
    ...
    assert len(df_summary) == 2, (
        "n_ortho=1 should yield 2 rows (p1 and o1)."
    )


def test_get_model_info_df(
    dummy_data: tuple[pd.DataFrame, pd.Series],
) -> None:
    """Validate global model metadata export."""
    df_X, df_y = dummy_data
    model = OPLSDA(n_ortho=1, cv_folds=3)
    model.fit(df_X, df_y)
...
def test_features_df_and_vip_ranking(
    dummy_data: tuple[pd.DataFrame, pd.Series],
) -> None:
    """Validate feature table columns and VIP ranking."""
    df_X, df_y = dummy_data
    model = OPLSDA(n_ortho=1, cv_folds=3)
    model.fit(df_X, df_y)
...
def test_scores_df_metadata(
    dummy_data: tuple[pd.DataFrame, pd.Series],
) -> None:
    """Validate sample score table metadata columns."""
    df_X, df_y = dummy_data
    model = OPLSDA(n_ortho=1, cv_folds=3)
    model.fit(df_X, df_y)
...
def test_permutation_test(
    ...
    """Validate permutation test output keys and lengths."""
    df_X, df_y = dummy_data
    model = OPLSDA(n_ortho=1, cv_folds=3)
    model.fit(df_X, df_y)
...
def test_sacurine_example_data() -> None:
    """Validate the end-to-end Sacurine workflow and diagnostic plot export."""
